...
# parser_utils.py - добавим эти функции
...
#         # BreedArchive часто использует пробелы: "RU CH" или "TH CH"
#         # Не делим по пробелам, а обрабатываем целиком
#         raw_titles = []
#
...
#         raw_titles = [t.strip() for t in text.split(',') if t.strip()]
#
#     titles_data = []
#
...
#             continue
#
#         # Проверяем, не является ли это кодом страны (RU, UA, BY и т.д.)
#         # Коды стран обычно короткие (2-3 буквы) и без точек
#         if len(raw_title) <= 3 and raw_title.isupper() and '.' not in raw_title:
#             # Это может быть код страны, пропускаем как отдельный титул
#             continue
#
#         title_info = parse_single_title(raw_title, source)
#         if title_info:
...
#
#
# def parse_single_title(raw_title: str, source: str = "zooportal") -> Optional[Dict[str, Any]]:
#     """
#     Парсит один титул из текста.
#
#     Поддерживает форматы:
#     - "GrCH.RUS" (Zooportal)
#     - "TH CH" (BreedArchive)
#     - "Чемпион России" (русский текст)
#     """
#     if not raw_title:
#         return None
#
...
#
#         # Титулы из BreedArchive
#         'th ch': ("TH CH", "Трехкратный Чемпион", True),
#         'tch': ("TCH", "Трехкратный Чемпион", True),
#         'triple champion': ("TCH", "Трехкратный Чемпион", True),
#
#         # Сертификаты
...
#
#     # 1. Ищем код страны после точки или пробела
#     patterns = [
#         r'\.([A-Z]{2,4})\b',  # GrCH.RUS
#         r'\s([A-Z]{2,4})\b',  # CH.CL RUS
#         r'\(([A-Z]{2,4})\)',  # (RUS)
#     ]
#
...
#             # Проверяем, что это не часть титула
#             if country not in ['CH', 'CL', 'CAC', 'CACIB', 'INT', 'EU', 'WORLD']:
#                 return country
#
#     # 2. Ищем по названиям стран
#     country_mapping = {
#         'РОССИИ': 'RUS',
#         'RUS': 'RUS',
#         'РФ': 'RUS',
...
#     }
#
#     for ru_name, code in country_mapping.items():
#         if ru_name in title_upper:
#             return code
#
#     return None
#
#
# def format_titles_for_db(titles_data: List[Dict[str, Any]]) -> Dict[str, Any]:
#     """
#     Форматирует данные титулов для сохранения в БД.
#     Возвращает словарь с полями:
#     - prefix_titles: строка с префиксными титулами
#     - suffix_titles: строка с суффиксными титулами
#     - other_titles: строка со всеми титулами
#     - titles_list: оригинальный список титулов (для JSON поля если нужно)
#     """
#     if not titles_data:
#         return {
#             'prefix_titles': '',
...
#         raw_text = title.get('raw_text', '')
#         is_prefix = title.get('is_prefix', True)
#
#         if raw_text:
#             other_titles.append(raw_text)
#
#             if is_prefix:
#                 prefix_titles.append(title.get('short_name', raw_text))
#             else:
#                 suffix_titles.append(title.get('short_name', raw_text))
#
#     return {
#         'prefix_titles': ', '.join(prefix_titles) if prefix_titles else '',
#         'suffix_titles': ', '.join(suffix_titles) if suffix_titles else '',
#         'other_titles': ', '.join(other_titles) if other_titles else '',
#         'titles_list': titles_data
#     }
...
#     Обрабатывает форматы: "RU CH", "UA CH", "TH CH", "RU JCH" и т.д.
#     """
#     if not raw_title:
#         return None
#
...
#
#     if len(words) == 2:
#         # Формат: "КОД ТИП" (например: "RU CH", "UA JCH", "TH CH")
#         first_word = words[0].upper()
#         second_word = words[1].upper()
#
#         # Определяем тип титула и страну
#         title_type = second_word
#         country_code = None
#
#         # Проверяем, является ли первое слово кодом страны
#         country_codes = {
...
#         elif first_word in ['TH', 'TCH', 'TRIPLE']:
#             # Это специальные титулы без страны
#             title_type = "TCH"
#             country_code = None
#         else:
#             # Неизвестный формат
...
#             is_prefix = True
#             country_code = None  # Для TCH не сохраняем страну
#
#         else:
...
#
#     else:
#         # Не формат "КОД ТИП", используем общий парсер
#         return parse_zooportal_title(raw_title)
def parse_breedarchive_title(raw_title: str) -> Optional[Dict[str, Any]]:
    """
    Специальный парсер титулов для BreedArchive.
...
def format_titles_for_db(titles_data: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    Для префиксных титулов формирует строку в формате: "CH.RUS, CH.UA, GrCH"
    Если есть страна - добавляет ее через точку.
    """
    if not titles_data:
        return {
            'prefix_titles': '',
    ...
    for title in titles_data:
        raw_text = title.get('raw_text', '')
        short_name = title.get('short_name', '')
        country = title.get('country')
        is_prefix = title.get('is_prefix', True)
        ...
        if raw_text:
            ...
            if is_prefix:
                # Формируем отображаемое название для префиксного титула
                if country and short_name:
                    display_name = f"{short_name}.{country}"
                else:
                    display_name = short_name
                prefix_titles.append(display_name)
            else:
                suffix_titles.append(short_name)
